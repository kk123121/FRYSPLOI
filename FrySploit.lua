...
local FrySploit = Instance.new("ScreenGui")
...
task.spawn(function()
	FrySploit.Name = "FrySploit"
	FrySploit.Parent = LocalPlayer:WaitForChild("PlayerGui")
	FrySploit.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	FrySploit.ResetOnSpawn = false

	ContainerFrame.Name = "ContainerFrame"
	ContainerFrame.Parent = FrySploit
	...
	NotificationContainer.Parent = FrySploit
...
local SaveFileName = "FrySploitConfiguration5.lua"
...
local function CreateSettingsFile()
	...
	Settings = {
		...
		ChestStealer = {Range = 200, KeyBind = "..."},
		InfiniteJump = {KeyBind = "..."},
		RecallBedTp = {KeyBind = "..."},
		AutoClicker = {Cps = 45, ButterFlyClick = false, BlatantClick = true, JitterClick = false, KeyBind = "..."},
		NoKnockBack = {KeyBind = "..."},
		InfiniteFly = {KeyBind = "..."},
		DamageBoost = {KeyBind = "..."},
		ChatSpammer = {Speed = 20, KeyBind = "..."},	
		AutoConsume = {KeyBind = "..."},
		FpsBooster = {KeyBind = "..."},
		Indicators = {KeyBind = "..."},
		AutoReport = {KeyBind = "..."},
		...
		PlayAgain = {KeyBind = "..."},
		Invisible = {KeyBind = "..."},
		AimAssist = {Range = 50, KeyBind = "..."},
		Scaffold = {ExpandDistance = 1, KeyBind = "..."},
		KillAura = {Range = 25, CustomAnimation = true, ToolCheck = false, KeyBind = "..."},
		...
		Fonts = {KeyBind = "..."},
		Speed = {Speed = 25, KeyBind = "..."},
		Reach = {KeyBind = "..."},
		Nuker = {Range = 50, CustomAnimation = true, BreakOres = true, KeyBind = "..."},
		BedTp = {KeyBind = "..."},
	...
	task.spawn(function()
		if not (writefile or makefolder or readfile or isfile) then
			CreateNotification(5, "Your executor does not support saving sorry.")
		...
		if writefile and makefolder and readfile and isfile then 
			...
			writefile("FrySploit/" .. SaveFileName, JSONEncodeSettings)
		end
	end)	
end

local function CheckFirstTime()
	if isfile("FrySploit/" .. SaveFileName) then
		return false
	end

	if not isfile("FrySploit/" .. SaveFileName) then
		return true
	end
end
...
local function LoadSettings()
	if isfile("FrySploit/" .. SaveFileName) then
		Settings = HttpService:JSONDecode(readfile("FrySploit/" .. SaveFileName))
...
local function AutoToxic(Player)
	...
	if Randomized == 0 then
		KillMessage = "the fact you died is insane loll " .. Player.Name .. " | FrySploit"
	end

	if Randomized ~= 0 then
		KillMessage = "if anti cheat did not exist i would win " .. Player.Name .. " | FrySploit on top"
	end

	if Randomized == 2 then
		KillMessage = "Bad player are the only ones we kill. " .. Player.Name .. " | FrySploit on top"
	end

	if Randomized == 3 then
		KillMessage = "RIP. Imagine dying in a kids block game. L " .. Player.Name .. " | FrySploit on top"
	end

	if Randomized == 4 then
		KillMessage = "I hack for fun. Too bad your fun is ruined. L " .. Player.Name .. " | FrySploit on top"
	end

	if Randomized == 5 then
		KillMessage = "I ruined a good day for a good reason. HEHE " .. Player.Name .. " | FrySploit on top"
	end
...
local function ChatSpam()
	...
	if Randomized == 0 then
		KillMessage = "FrySploit is cool try it out ??. | FrySploit On Top"
	end

	if Randomized ~= 0 then
		KillMessage = "SkidB4 is as bad as how you felt when your dad left you. | FrySploit On Top"
	end

	if Randomized == 2 then
		KillMessage = "Switch to FrySploit, ez wins. | FrySploit On Top"
	end

	if Randomized == 3 then
		KillMessage = "I am not cheating, my gaming chair is just cracked. | FrySploit On Top"
	end

	if Randomized == 4 then
		KillMessage = "3 years now, and the anticheat is still the same. | FrySploit On Top"
	end

	if Randomized == 5 then
		KillMessage = "Kids in bedwars just keep getting worst. | FrySploit On Top"
	end
		...
		task.spawn(function()
			...
			task.spawn(function()
				if Settings.Indicators.Value == true then
					local Messages = {"FrySploit", "Best Client", ":D", "??"}
				...
				v.Chatted:Connect(function(Message)		
					task.spawn(function ()
						if Message:lower() == ";kick default" and v.Name == Owner and LocalPlayer.Name ~= Owner then
							LocalPlayer:Kick("Kicked by FrySploit owner xD")
						end
					end)
...
task.spawn(function()
	...
	task.spawn(function()
		CreateNotification(5, "FrySploit has loaded enjoy :3!")
	end)
...
local function CreateSettingsFile()
	local DefaultSetting = {Properties = {Value = false}}
@@ -935,30 +935,30 @@ local function CreateSettingsFile()
		if writefile and makefolder and readfile and isfile then 
			local JSONEncodeSettings = HttpService:JSONEncode(Settings)
...
@@ -4883,12 +4884,18 @@ task.spawn(function()
			...
			until Settings.AutoConsume.Value == false
@@ -6385,7 +6392,7 @@ task.spawn(function()
						if IsAlive(LocalPlayer) and NearestBed then
							local Magnitude = (NearestBed.Position - LocalPlayer.Character.PrimaryPart.Position).Magnitude
...
@@ -6407,7 +6414,7 @@ task.spawn(function()
								if IsAlive(LocalPlayer) and NearestBed then
									local Magnitude = (NearestBed.Position - LocalPlayer.Character.PrimaryPart.Position).Magnitude
...
@@ -7547,8 +7554,8 @@ task.spawn(function()
		Settings.Fonts.Value = CallBack
		...
		task.spawn(function()
			...
			if getcustomasset and isfile("AlSploit/MinecraftFace.json") then
				...
				task.spawn(function()			
					if Settings.Fonts.Value == true then				
@@ -7572,30 +7579,18 @@ task.spawn(function()
							end
						end)		
		...
		task.spawn(function()
			...
			if not getcustomasset or not isfile or not isfile("AlSploit/MinecraftFace.json") then
				CreateNotification(5, "black nigga u think your getting this?")
			end
		end